// mongo cdr cep3g_stat_phone.js  > ./log/agg_$(date +"%Y%m%d")_$(date +"%H%M%S").txt
...
var agg_3g = db.cep3g_join.aggregate([
        ...
        ,{$project:{
            ...
            //, HO_DISTINCT:{$cond :[{$and:[{$gt:["$HO",0]},{$eq:["$record_type","1" ]} ]},"$called_number",0]}
        }}
        ,{$group:{
            _id: {
                DATE : "$DATE"
